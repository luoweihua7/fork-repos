...
## 下载 RouterOS 镜像校验文件
$ curl -LR -O https://download.mikrotik.com/routeros/7.23.5/chr-7.23.5.img.zip.sha256

## 下载 RouterOS 镜像文件
$ curl -LR -O https://download.mikrotik.com/routeros/7.23.5/chr-7.23.5.img.zip
```
...
## 执行镜像文件校验
$ sha256sum --check chr-7.23.5.img.zip.sha256

#### 镜像校验成功示例输出
chr-7.23.5.img.zip: OK
```
...
```bash
## 将镜像文件解压缩
$ python3 -m zipfile -e chr-7.23.5.img.zip "$(pwd)"
```
...
```bash
## 将 img 镜像导入虚拟机中
$ qm importdisk 101 chr-7.23.5.img local-lvm
